...
//! # Design
//!
//! A SharedPipe uses a ring buffer in a SharedArrayBuffer:
//!
//! ```text
//! Offset  Size   Field
//! ──────  ────   ─────
...
/// the buffer and creates views on-demand.
#[derive(Debug)]
...
// Safety: In WASM, each Worker runs single-threaded. SharedArrayBuffer is explicitly
// designed to be shared across Workers, and we use Atomics for synchronization.
// The JsValue raw pointer in SharedArrayBuffer is safe because:
// 1. Each Worker creates its own typed array views (Int32Array, Uint8Array)
// 2. Only the SharedArrayBuffer reference is shared, not the views
// 3. All access to shared data uses Atomics operations
...
impl SharedPipeTx {
    /// Create an Int32Array view for atomic operations.
    #[inline]
    fn int32_view(&self) -> Int32Array {
        Int32Array::new(&self.buffer)
    }

    /// Create a Uint8Array view for data access.
    #[inline]
...
impl SharedPipe {
    ...
    /// Create a SharedPipe from an existing SharedArrayBuffer.
    ///
    ...
    ///
    /// This buffer can be passed to another Worker to establish communication.
...
impl SharedPipeTx {
    ...
    fn free_space(&self) -> usize {
        ...
        if write_pos >= read_pos {
            self.data_capacity - (write_pos - read_pos) - 1
        } else {
    ...
    pub fn write_data(&mut self, data: &[u8]) -> io::Result<usize> {
        ...
        let first_chunk_size = (self.data_capacity - write_pos).min(to_write);
        ...
        for i in 0..first_chunk_size {
            uint8_view.set_index((HEADER_SIZE + write_pos + i) as u32, data[i]);
        }

        // Second chunk: from start of buffer (wrap-around)
        for i in 0..second_chunk_size {
            uint8_view.set_index((HEADER_SIZE + i) as u32, data[first_chunk_size + i]);
        }

        // Update write position
        let new_write_pos = (write_pos + to_write) % self.data_capacity;
...
impl SharedPipeRx {
    ...
    fn data_available(&self) -> usize {
        ...
        if write_pos >= read_pos {
            ...
            self.data_capacity - read_pos + write_pos
        }
    }
    ...
    pub fn try_read(&mut self, buf: &mut [u8]) -> Option<usize> {
        ...
        let first_chunk_size = (self.data_capacity - read_pos).min(to_read);
        let second_chunk_size = to_read - first_chunk_size;

        // First chunk: from read_pos to end of buffer (or end of request)
        for i in 0..first_chunk_size {
            buf[i] = uint8_view.get_index((HEADER_SIZE + read_pos + i) as u32);
        }

        // Second chunk: from start of buffer (wrap-around)
        for i in 0..second_chunk_size {
            buf[first_chunk_size + i] = uint8_view.get_index((HEADER_SIZE + i) as u32);
        }

        // Update read position
        let new_read_pos = (read_pos + to_read) % self.data_capacity;
...
impl SharedStdioPipes {
    ...
    /// Get the SharedArrayBuffers for the child side.
    ///
    /// Returns (stdin_buffer, stdout_buffer, stderr_buffer).
    /// The child will:
    /// - Read from stdin_buffer
    /// - Write to stdout_buffer
    /// - Write to stderr_buffer
    pub fn child_buffers(&self) -> (SharedArrayBuffer, SharedArrayBuffer, SharedArrayBuffer) {
        (
            self.stdin.buffer().clone(),
            self.stdout.buffer().clone(),
            self.stderr.buffer().clone(),
        )
    }
    ...
    /// Create child-side pipes from SharedArrayBuffers.
    ///
    /// This is used on the child worker to connect to the parent's pipes.
    pub fn child_from_buffers(
        stdin_buffer: SharedArrayBuffer,
        stdout_buffer: SharedArrayBuffer,
        stderr_buffer: SharedArrayBuffer,
    ) -> Self {
        Self {
            stdin: SharedPipe::from_buffer(stdin_buffer),
            stdout: SharedPipe::from_buffer(stdout_buffer),
            stderr: SharedPipe::from_buffer(stderr_buffer),
        }
    }
